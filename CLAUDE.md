...
The vault lives in the sibling repo `~/Documents/GitHub/notes`, not here. Edit notes there — `src/data/notes/` in this repo is a gitignored copy regenerated by `scripts/sync-notes.js` on every `npm run dev`/`npm run build`. See `.claude/docs/notes.md`.
